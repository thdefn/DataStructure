...
public class GeneticAlgorithm {
    ...
    public static int localOptimizedCount = 0;
    public static int localOptimizedThreshold = 900;
    ...
    public static void main(String[] args) throws IOException {
        ...
        bw = new BufferedWriter(new FileWriter("src/algorithm/island/graph/chimera.out"));
        ...
        while (trial-- > 0) {
            ...
            for (Island island : islands) {
                island.localOptimized(0.1);
            }

            int max = Collections.max(islands.get(0).improvedFitnesse) + 1;

            List<List<Integer>> datasets = new ArrayList<>();
            for (Island island : islands) {
                System.out.println(island.getBestIndividual().fitness);
                datasets.add(island.improvedFitnesse.stream().map(f -> max - f).collect(Collectors.toList()));
            }
            ...
            System.out.println("-------------------------after migrated-------------------------");
            ...
            for (Island island : islands) {
                island.localOptimized(0.01);
            }

            Island best = islands.get(0);
            bestResult = islands.get(0).getBestIndividual();
            for (Island i : islands) {
                if (i.getBestIndividual().fitness > bestResult.fitness) {
                    bestResult = i.getBestIndividual();
                    best = i;
            ...
            datasets.add(best.improvedFitnesse.stream().map(f -> max - f).collect(Collectors.toList()));
            JFrame frame = new JFrame("GA Island Local Optimization Result");
            MultiGraphPanel multiGraphPanel = new MultiGraphPanel(datasets);
            multiGraphPanel.setPreferredSize(new Dimension(800, 600));
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.getContentPane().add(multiGraphPanel);
            frame.pack();
...
class Individual implements Comparable<Individual> {
    ...
    public int fitnessToCost(int max){
        return max - fitness;
    }
    ...
    public void localOptimized(List<Integer> improvedFitnesse) {
        System.out.println(this.fitness);
        improvedFitnesse.add(this.fitness);
        System.out.println("trial : " + GeneticAlgorithm.localOptimizedCount++);
...
class Island {
    public List<Integer> improvedFitnesse = new ArrayList<>();
    ...
    public void evolve() {
        ...
        for (int j = 0; j < POPULATION_SIZE; j++) {
            ...
            Individual child = ccrossover(p1, p2);
            mutate(child);
...
//            improvedFitnesse = new ArrayList<>();
//            localOptimized(0.1);
//        }
    }

    public void localOptimized(double prop){
        getBestIndividual().localOptimized(improvedFitnesse);
        for (int i = 0; i < POPULATION_SIZE * prop; i++) {
            population.set(i, getBestIndividual().copy());
        }
        noImprovementCount = 0;
        bestFitness = getBestIndividual().fitness;
    }
