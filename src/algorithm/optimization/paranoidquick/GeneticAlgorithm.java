package algorithm.optimization.paranoidquick;
...
public class GeneticAlgorithm {
    ...
    public static void main(String[] args) throws IOException {
        ...
        BufferedWriter bw = new BufferedWriter(new FileWriter("src/algorithm/optimization/paranoidquick/maxcut.out"));
        ...
        while (trial-- > 0) {
            ...
            while (generationSize-- > 0) {
                ...
                paranoidQuickSort(population, 0, population.size() - 1);
             }
    ...
    private static void paranoidQuickSort(List<Individual> array, int begin, int end) {
        if (begin < end) {
            int pivot = findMedian(array, begin, end);
            int part = partition(array, begin, end, pivot);
            paranoidQuickSort(array, begin, part - 1);
            paranoidQuickSort(array, part + 1, end);
        }
    }

    private static int findMedian(List<Individual> arr, int left, int right) {
        PriorityQueue<Integer> minHeap = new PriorityQueue<>(); // min -> max
        PriorityQueue<Integer> maxHeap = new PriorityQueue<>(Collections.reverseOrder()); // max -> min
        for (int i = left; i <= right; i++) {
            if (minHeap.size() == maxHeap.size()) {
                maxHeap.offer(arr.get(i).fitness);
                minHeap.offer(maxHeap.poll());
            } else {
                minHeap.offer(arr.get(i).fitness);
    ...
    private static int partition(List<Individual> array, int begin, int end, int pivot) {
        int i = (begin - 1);
