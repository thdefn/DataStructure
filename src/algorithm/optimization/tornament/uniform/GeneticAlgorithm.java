package algorithm.optimization.tornament.uniform;
...
public class GeneticAlgorithm {
    ...
    public static void main(String[] args) throws IOException {
        System.setIn(new FileInputStream("src/algorithm/optimization/tornament/uniform/maxcut.in"));
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        BufferedWriter bw = new BufferedWriter(new FileWriter("src/algorithm/optimization/tornament/uniform/kpoint.out"));
...
//            }
    ...
    public static Individual crossover(Individual p1, Individual p2) {
        int length = Math.min(p1.vertexes.size(), p2.vertexes.size());
        List<Integer> c1 = new ArrayList<>();
        for (int i = 0; i < length; i++) {
            if (random.nextBoolean())
                c1.add(p1.vertexes.get(i));
            else c1.add(p2.vertexes.get(i));
        }
