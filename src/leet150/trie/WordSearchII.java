...
public class WordSearchII {
    ...
    private static ArrayList<TreeNode> isExistedInNode(char[] chars, int start, int end) {
        if(start == end){
            if(!root.children.containsKey(chars[start])) return new ArrayList<>();
            for (TreeNode node:
            root.children.get(chars[start]).values()) {
                
            }
        }
        int mid = (start + end) / 2;
        isExistedInNode(chars, start, mid);
        isExistedInNode(chars, mid + 1, end);
        return null;
    }
